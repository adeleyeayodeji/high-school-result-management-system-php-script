...
{   ?>
     <h2 class="title" align="center"><?php echo htmlentities($row->StudentName);?> Exam Result</h2>

<?php }

    ?>
            <center><p class="btn btn-primary" style="text-align:center;background: black;color: white;">Year of Examination : 
                                   <?php
                             $termid = $row->StudentId;
                             $query = mysqli_query($con, "SELECT year FROM tblresult WHERE StudentId='$termid' ") or die();
                             $resultterm = mysqli_fetch_assoc($query);
                             echo $resultterm["year"];
                             ?>
                            </p></center>
...
<?php
 ...
 $resultterm = mysqli_fetch_assoc($query);
 echo $resultterm["term"];
 ?>
<p><b>Student Department :</b> <?php echo htmlentities($row->Departments);?>
...
<tr>
                                                <th scope="row" colspan="2">Print Result</th>           
                                                            <td><b><a href="javascript:void()" class="btn btn-primary" style="text-align:center;background: black;color: white;    font-size: 10px;" onclick="window.print();">Print </a> </b></td>
                                                             </tr>
...
echo "<a href='find-result.php' class='btn btn-primary' style='float: right;background: black;color: white;'>
                                                    Check again
                                            </a>";
 }
?>
